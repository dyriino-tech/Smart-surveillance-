...
# Replace with your own Telegram Bot credentials
...
def send_telegram(photo_path):
    try:
        ...
        message = ("ALERT! Motion detected at " + timestamp +
                   "\nLocation: " + location +
                   "\nSmart Surveillance - Dyriino Tech")
        url = "https://api.telegram.org/bot" + BOT_TOKEN + "/sendPhoto"
        with open(photo_path, "rb") as photo:
            response = requests.post(url, data={"chat_id": CHAT_ID, "caption": message}, files={"photo": photo})
...
while True:
    ...
    if motion:
        ...
        counter = 0
    status = 'MOTION DETECTED - ALERT SENT!' if motion else 'Monitoring...'
